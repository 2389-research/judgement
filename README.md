...
question. Use `--` if the question starts with a dash. Running `judgement` with no
arguments prints the help text, the same as `--help`.
...
Stable releases also update `Casks/judgement.rb` in
...
to that tap. Prereleases leave the stable cask unchanged. The cask installs the
prebuilt macOS binary for amd64 or arm64; Linux users download the release
archive directly. The generator checks each macOS archive against its release
checksum before emitting the cask.

After building a snapshot, validate cask generation without a token:

```sh
go run ./internal/cmd/homebrew-cask --dist dist > dist/judgement.rb
ruby -c dist/judgement.rb
```

CI runs this same generation and syntax check. It does not install the cask: an
unsigned macOS binary can't clear Gatekeeper in headless CI.

After the first stable release publishes the cask, install on macOS with
`brew install --cask 2389-research/tap/judgement`. CI validates generation; the
first tagged release will also verify the stored token's write access to the tap.
